...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`[TURMAS API] Retornando ${turmas.length} turmas`);
        // Desabilitar cache para sempre retornar dados frescos
...
router.get('/:id/alunos', authenticateToken, async (req, res) => {
    try {
        const alunos = await db.query(`
            ...
            LEFT JOIN graduacoes_sistema g ON al.graduacao_atual = g.id
            WHERE at.turma_id = ?
            ORDER BY u.nome
        `, [req.params.id]);
...
// Criar nova turma
router.post('/', authenticateToken, authorize('admin', 'professor'), async (req, res) => {
    try {
        const {
            nome,
            professor_id,
        ...
        const professorIdValue = professor_id === '' || professor_id === undefined ? null : professor_id;
        const graduacaoMinimaIdValue = graduacao_minima_id === '' || graduacao_minima_id === undefined ? null : graduacao_minima_id;
        const graduacaoMaximaIdValue = graduacao_maxima_id === '' || graduacao_maxima_id === undefined ? null : graduacao_maxima_id;
        ...
        const result = await db.query(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [nome, professorIdValue, graduacaoMinimaIdValue, graduacaoMaximaIdValue, programa, dia_semana, horario_inicio, horario_fim, capacidade_maxima]);
...
// Atualizar turma
router.put('/:id', authenticateToken, authorize('admin', 'professor'), async (req, res) => {
    try {
        const {
            nome,
            professor_id,
        ...
        await db.query(`
            ...
            WHERE id = ?
        `, [nome, professorIdValue, graduacaoMinimaIdValue, graduacaoMaximaIdValue, programa, dia_semana, horario_inicio, horario_fim, capacidade_maxima, status, req.params.id]);
...
router.delete('/:id', authenticateToken, authorize('admin', 'professor'), async (req, res) => {
    try {
