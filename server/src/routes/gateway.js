...
let mpClient, preferenceClient, paymentClient;
...
router.post('/mercadopago/create-preference', authenticateToken, async (req, res) => {
  try {
    ...
    logger.info(`Preferência Mercado Pago criada: ${response.id} para usuário ${userId}`);
...
router.post('/mercadopago/create-pix', authenticateToken, async (req, res) => {
  try {
    ...
    logger.info(`PIX Mercado Pago criado: ${response.id} para usuário ${userId}`);
