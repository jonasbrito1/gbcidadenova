...
ADD COLUMN primeiro_acesso BOOLEAN DEFAULT TRUE COMMENT 'Indica se é o primeiro acesso do usuário';

ALTER TABLE usuarios
ADD COLUMN lgpd_aceite BOOLEAN DEFAULT FALSE COMMENT 'Indica se usuário aceitou termos LGPD';

ALTER TABLE usuarios
ADD COLUMN lgpd_aceite_data TIMESTAMP NULL COMMENT 'Data e hora do aceite LGPD';

ALTER TABLE usuarios
ADD COLUMN lgpd_aceite_ip VARCHAR(45) NULL COMMENT 'IP do aceite LGPD';

ALTER TABLE usuarios
ADD COLUMN reset_token VARCHAR(255) NULL COMMENT 'Token para reset de senha';

ALTER TABLE usuarios
ADD COLUMN reset_token_expiry TIMESTAMP NULL COMMENT 'Expiração do token de reset';

-- Adicionar índices para melhor performance
CREATE INDEX idx_reset_token ON usuarios(reset_token);
CREATE INDEX idx_primeiro_acesso ON usuarios(primeiro_acesso);
...
ALTER TABLE alunos
ADD COLUMN cep VARCHAR(10) NULL COMMENT 'CEP do endereço';

ALTER TABLE alunos
ADD COLUMN rua VARCHAR(255) NULL COMMENT 'Rua/Logradouro';

ALTER TABLE alunos
ADD COLUMN numero VARCHAR(20) NULL COMMENT 'Número do endereço';

ALTER TABLE alunos
ADD COLUMN complemento VARCHAR(100) NULL COMMENT 'Complemento do endereço';

ALTER TABLE alunos
ADD COLUMN bairro VARCHAR(100) NULL COMMENT 'Bairro';

ALTER TABLE alunos
ADD COLUMN cidade VARCHAR(100) NULL COMMENT 'Cidade';

ALTER TABLE alunos
ADD COLUMN estado VARCHAR(2) NULL COMMENT 'Estado (UF)';

-- Adicionar campo de email do responsável (para menores de idade)
ALTER TABLE alunos
ADD COLUMN email_responsavel VARCHAR(255) NULL COMMENT 'Email do responsável (obrigatório para menores)';

-- Atualizar contato de emergência para incluir nome
ALTER TABLE alunos
ADD COLUMN nome_contato_emergencia VARCHAR(255) NULL COMMENT 'Nome do contato de emergência';

-- Adicionar campos médicos detalhados
ALTER TABLE alunos
ADD COLUMN tipo_sanguineo VARCHAR(5) NULL COMMENT 'Tipo sanguíneo (A+, A-, B+, B-, AB+, AB-, O+, O-)';

ALTER TABLE alunos
ADD COLUMN toma_medicamento BOOLEAN DEFAULT FALSE COMMENT 'Indica se toma medicamento regularmente';

ALTER TABLE alunos
ADD COLUMN medicamentos_detalhes TEXT NULL COMMENT 'Detalhes dos medicamentos que toma';

ALTER TABLE alunos
ADD COLUMN historico_fraturas BOOLEAN DEFAULT FALSE COMMENT 'Indica se já teve fraturas';

ALTER TABLE alunos
ADD COLUMN fraturas_detalhes TEXT NULL COMMENT 'Detalhes das fraturas (membros, datas)';

ALTER TABLE alunos
ADD COLUMN tem_alergias BOOLEAN DEFAULT FALSE COMMENT 'Indica se possui alergias';

ALTER TABLE alunos
ADD COLUMN alergias_detalhes TEXT NULL COMMENT 'Detalhes das alergias';

-- Adicionar índices para campos de busca
CREATE INDEX idx_alunos_cep ON alunos(cep);
CREATE INDEX idx_alunos_email_responsavel ON alunos(email_responsavel);
